...
class CuratedItem:
    ...
    Attributes:
        ...
        reason: Explanation of why this item was recommended (generated by LLM)
    """
...
class CuratorAgent(BaseAgent):
    ...
    def rank_items(
        ...
        Returns:
            Ranked list of CuratedItem with scores and reasons populated.
        """
        ...
        # Generate reasons for top items
        for item in top_items:
            try:
                item.reason = self.generate_item_reason(item, profile)
            except Exception as e:
                self._log_error(
                    f"Failed to generate reason for item: {item.title}", exception=e
                )
                # Leave reason as None, will fallback to summary in templates
...
"""
    ...
    def generate_item_reason(
        self, item: CuratedItem, profile: UserProfileSettings
    ) -> str:
        """
        Generate a short explanation (1-2 sentences) of why this item was recommended.

        Uses LLM to create a personalized reason based on item content and user preferences.

        Args:
            item: The CuratedItem to generate a reason for.
            profile: User profile with preferences.

        Returns:
            A short reason string, or fallback message if generation fails.
        """
        prefs_desc = ""
        if profile.topics:
            prefs_desc += f"- Preferred topics: {', '.join(profile.topics)}\n"
        ...
        item_summary_preview = (
            item.summary[:200] + "..." if len(item.summary) > 200 else item.summary
        ) if item.summary else "No summary available"

        prompt = f"""You are an AI assistant helping to explain why a specific content item was recommended to a user.
...
Recommended item:
- Title: {item.title}
- Source: {item.source_type} ({item.provider or 'Unknown provider'})
- Relevance score: {item.score:.2f}
- Summary: {item_summary_preview}

Write a brief, friendly explanation (1-2 sentences) explaining why this specific item was recommended for this user.
Focus on how it matches their interests, preferred topics, or providers. Be concise and specific.
Do not use emojis.
"""
        ...
        try:
            reason = self._call_llm(messages=messages, temperature=0.7, max_tokens=150)
            return reason.strip() or "This item matches your interests."
        except Exception as e:
            self._log_error(
                f"Failed to generate reason for item: {item.title}", exception=e
            )
            return "This item matches your interests."
