"""
Test đơn giản cho OpenAIScraper - không cần database hay config.
...
# Copy các hàm utility từ OpenAIScraper để test độc lập
...
def test_extract_article_id():
    ...
    test_cases = [
        ("https://openai.com/blog/chatgpt", "blog/chatgpt"),
        ("https://openai.com/research/gpt-4", "research/gpt-4"),
        ("https://openai.com/blog/", "blog"),
        ("https://openai.com/", "https://openai.com/"),  # Fallback case
    ]
...
def test_parse_rss_feed():
    """Test parse RSS feed từ OpenAI blog"""
    print("🧪 Test: Parse RSS Feed từ OpenAI Blog")
    print("-" * 60)
    ...
    if feed and hasattr(feed, 'entries'):
        ...
        if feed.entries:
            ...
            # Test extract author if available
            if hasattr(first_article, 'author'):
                print(f"   Author: {first_article.author}")
            elif hasattr(first_article, 'authors') and first_article.authors:
                print(f"   Author: {first_article.authors[0].get('name', 'N/A')}")
...
def main():
    ...
    print("=" * 60)
    print("🚀 BẮT ĐẦU TEST OPENAI SCRAPER (Đơn giản)")
    print("=" * 60)
    print()
